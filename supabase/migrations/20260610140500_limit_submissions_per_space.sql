-- Limit each anonymous diagnostic space to 300 complete submissions.
-- The limit is enforced inside the atomic server-only submission RPC.
...
comment on function public.create_submission_with_answers(text, text, jsonb) is
  'Server-only RPC that validates and inserts one anonymous complete submission atomically, capped at 300 submissions per diagnostic space.';
