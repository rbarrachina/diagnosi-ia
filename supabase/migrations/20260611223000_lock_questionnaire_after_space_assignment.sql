-- Lock questionnaire content as soon as a diagnostic space uses the version.
-- Waiting for submissions is too late because the public questionnaire may
-- already have been shared.

create or replace function private.questionnaire_space_count(
...
as $$
  ...
  where diagnostic_spaces.questionnaire_id = p_questionnaire_id;
$$;
...
begin
  ...
  if private.questionnaire_space_count(p_questionnaire_id) <> 0 then
    raise exception 'Questionnaire content cannot be changed after diagnostic spaces exist'
      using errcode = '23514';
  end if;
...
comment on function private.questionnaire_space_count(text) is
  'Counts diagnostic spaces using a questionnaire version without exposing rows to browser clients.';
comment on function public.replace_questionnaire_content(text, text, jsonb) is
  'Server-only replacement of questionnaire blocks and questions before any diagnostic space uses the version.';
